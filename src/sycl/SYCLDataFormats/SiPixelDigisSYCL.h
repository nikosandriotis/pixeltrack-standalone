...
public:
  ...
  public:
    // DeviceConstView() = default;
     ...
     __inline__ uint16_t xx(int i) const { return *(xx_ + i); }
     __inline__ uint16_t yy(int i) const { return *(yy_ + i); }
     __inline__ uint16_t adc(int i) const { return *(adc_ + i); }
     __inline__ uint16_t moduleInd(int i) const { return *(moduleInd_ + i); }
     __inline__ int32_t clus(int i) const { return *(clus_ + i); }
